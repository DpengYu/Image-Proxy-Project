...
# 1. 安装/升级 Python 包
########################################
echo "[STEP 1] 安装/升级 Python 包 (venv)"
...
# 2. 系统工具检查
########################################
echo "[STEP 2] 检查系统命令 (nginx, openssl, jq)"
...
# 3. 日志目录
########################################
echo "[STEP 3] 创建日志目录 /var/log/image_proxy"
...
# 5. systemd 服务
########################################
echo "[STEP 4] 配置 systemd 服务 (fastapi + cleanup)"
...
# 6. Nginx 配置（宝塔或独立）
########################################
echo "[STEP 5] 配置 Nginx 反向代理 /docs /upload /get /secure_get /download_db"
...
if [ -f "$BT_CONF" ]; then
  ...
  echo "[INFO] 未检测到宝塔配置，创建/更新独立配置 $NGINX_CONF"
...
# 9. 验证
########################################
echo "[STEP 8] 测试 FastAPI 本地与域名 /docs"
sleep 2
...
echo "==> install.sh 完成"
echo "验证方法:"
echo "1. curl http://$DOMAIN/secure_get/<md5>?token=<token>"
echo "2. sqlite3 $PROJECT_DIR/server/images.db \"SELECT md5,access_count FROM images;\""
